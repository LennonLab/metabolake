...
# Used muscle on aligned files to build maximum likelihood tree
...
# there are other trees outputted: 
# UL.ml.raxml.bestTree (best ML tree), UL.ml.raxml.support (best tree + bootstrap support)
# UL.ml.raxml.bootstraps (all bootstrap replicates), UL.ml.raxml.bestModel (optimized model parameters)
...
# Mistake made in input fasta file with sequence naming
# rename Surf1IUB2501 -> Surf1
#pos <- which(tree$tip.label == "Surf1IUB2501")
#if (length(pos) == 1) {
#  tree$tip.label[pos] <- "Surf1"
#} else {
#  warning("Label Surf1IUB2501 not found or found multiple times")
#} # if not found, we just continue
...
png(filename="~/GitHub/metabolake/figures/FigTREE.png",
...
# Show Plot
img <- readPNG("~/GitHub/metabolake/figures/FigTREE.png")
